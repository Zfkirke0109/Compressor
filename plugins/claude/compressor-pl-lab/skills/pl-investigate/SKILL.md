...
Apply the bundled constitution. Investigation explains a captured verdict; it does not issue a replacement verdict or benchmark report.
...
1. Dry-run first:
   ...
   `python -B "${CLAUDE_PLUGIN_ROOT}/scripts/pl_lab.py" workflow investigate --repository "<repo>" --request "<request.json>" --experiment "<dir>"`
...
Native invocation: `/compressor-pl-lab:pl-investigate`.
