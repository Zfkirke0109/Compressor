...
Apply the bundled `android-media3-perceptual-compression` constitution. Never loosen frozen constraints after seeing candidate results.
...
1. Resolve the [bundled CLI](../../scripts/pl_lab.py), then dry-run before mutation or expensive work:

   `python -B "<resolved-script>" workflow benchmark --repository "<repo>" --request "<request.json>" --experiment "<dir>"`
...
Codex invocation: `$compressor-pl-lab:pl-benchmark`. In ChatGPT, type `@` and select this Skill. ChatGPT can guide or inspect supplied evidence, but it does not gain local repository/device/Python/host-binary access and does not execute Codex hooks.
