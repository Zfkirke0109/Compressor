...
1. Resolve the [bundled CLI](../../scripts/pl_lab.py), then dry-run:
   ...
   `python -B "<resolved-script>" workflow calibrate --repository "<repo>" --request "<request.json>" --experiment "<dir>"`
...
Codex invocation: `$compressor-pl-lab:pl-calibrate`. In ChatGPT, type `@` and select this Skill. ChatGPT can guide the workflow or inspect supplied evidence, but it does not gain local repository/device/Python/host-binary access and does not execute Codex hooks.
