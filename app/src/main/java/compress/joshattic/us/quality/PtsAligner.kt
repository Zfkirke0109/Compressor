 ...
 *
 * When the heads disagree by more than the tolerance, the EARLIER head is dropped so the
 * streams can re-align (measured on device: the Transformer probe clip and the reference
 * window reader disagree by exactly one frame about where a window starts — capture
 * batch_20260716_185345, skew 33.2 ms constant at 29.97 fps). Drops are budgeted; when
 * the budget is exhausted the window has no honest pixel evidence and the caller must
 * fail closed. Misalignment is never scored as quality.
 */
class PtsAligner(
    ...
    fun decide(refNormUs: Long, distNormUs: Long): Action {
        ...
        if (kotlin.math.abs(skew) <= toleranceUs()) return Action.PAIR
        if (refDropped + distDropped >= maxDrops) return Action.FAIL
        return if (skew > 0) {
